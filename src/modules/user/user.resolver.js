module.exports = {
	Query: {
		...
		user(_, { id }, { loaders: { userLoaders }, services: { userService } }) {
			return userLoaders().userById.load(id);
		},
		me(_, __, { loaders: { userLoaders }, req }) {
			return userLoaders().userById.load(req.userId);
		},
		usersById(_, { ids }, { loaders: { userLoaders } }) {
			return userLoaders().usersById.loadMany(ids);
		},
		photoLikers(_, { ids }, { loaders: { userLoaders } }) {
			return userLoaders().usersById.loadMany(ids);
		},
	},
